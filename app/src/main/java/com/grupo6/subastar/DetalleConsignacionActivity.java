    ...
    private void pintar(ConsignacionDTO c) {
        ...
        if (esVendida) {
            ...
            if (c.getImporteFinalObtenido() != null) {
                tvImporteFinal.setText(
                        "Importe final obtenido: "
                                + importe(c.getCondicionesEmpresa() != null
                                        ? c.getCondicionesEmpresa().getMoneda()
                                        : null,
                                c.getImporteFinalObtenido())
                                + "\n(precio de venta menos 15% de comision)");
            } else {
                tvImporteFinal.setText("El pago del comprador esta pendiente.");
            }
        } else {
